...
class ParallelQueryOutput(BaseModel):
...
class ParallelQuery(SingletonTemplate):
    """
    ...
    OutputSchema = ParallelQueryOutput
    ...
    def callback(self, result: List[TaskResult]) -> List[ParallelQueryOutput]:
        results = []
        for r in result:
            calls = get_tags(r.result, "function_calls")
